...
test_cfg = dict(
    ...
    min_bbox_size=0,
    score_thr=0.5,
    nms=dict(type='nms_rotated', iou_thr=0.5),
    max_per_img=1000)
# dataset settings
dataset_type = 'DeepScoresV2Dataset'
data_root = 'data/imslp_dataset/test/'
img_norm_cfg = dict(
    mean = [240, 240, 240],
    std = [57, 57, 57],
    to_rgb=False)
test_pipeline = [
    ...
    dict(
        ...
        flip=False,
        transforms=[
            dict(type='RotatedResize', img_scale=(980, 1900), keep_ratio=True),
...
data = dict(
    ...
    test=dict(
        type=dataset_type,
        ann_file=data_root + 'imslp_test.json',
        img_prefix=data_root + 'images/',
        pipeline=test_pipeline,
        use_oriented_bboxes=True))
# evaluation = dict(
...
wandb_cfg = dict(
    ...
    project='s2anet_uda',
    dryrun=False,
    name_prefix = "ghos_halfrez_crop_"
)
